...
decode_iter=
decode_nj=50
mfcc_hires_conf=mfcc_hires_80.conf

# training options
frames_per_eg=150,110,100
remove_egs=false
common_egs_dir=exp/chain/tdnn8c_sp/egs
xent_regularize=0.1
num_epochs=20

test_online_decoding=false  # if true, it will run the last decoding stage.
train_set=train_100k_nodup
...
ali_dir=exp/tri4_ali_100k_nodup$suffix
lat_dir=exp/tri4_lats_100k_nodup$suffix
treedir=exp/chain/tri5_${affix}_tree$suffix
...
local/nnet3/run_ivector_common.sh --stage $stage \
  ...
  --mfcc-hires-conf $mfcc_hires_conf \
  --generate-alignments $speed_perturb \
  --train-set $train_set --ali-dir $ali_dir || exit 1;
...
if [ $stage -le 9 ]; then
  ...
  #nj=$(cat exp/tri4_ali_100k_nodup$suffix/num_jobs) || exit 1;
...
if [ $stage -le 12 ]; then
  ...
  spec-augment-layer name=spec-augment freq-max-proportion=0.4 freq-max-regions=2 time-zeroed-proportion=0.01 time-mask-max-frames=2 include-in-init=true
  delta-layer name=delta
  no-op-component name=input2 input=Append(delta, Scale(1.0, ReplaceIndex(ivector, t, 0)))

  relu-batchnorm-layer name=tdnn1 $affine_opts dim=1536 input=input2
